...
from .models import EventDetail, Category, Version, Resource, Date
from .serializers import EventSerializer, CategorySerializer, ResourceSerializer, DateSerializer


def index(request):
    return JsonResponse("Hello, world. You're at the O-Week index. Get ready to be awed.",
                        safe=False)
...
def feed(request, day, req_category="NONE"):
    if request.method == 'GET':
        if req_category.lower() == "none":
            ...
            event_set = EventDetail.objects.filter(
                start_date__day=str(day))  # because its a five day event
        else:
...
def eventImage(request, event_id):
    ...
    response = HttpResponse(s3key.read(), status=status.HTTP_200_OK,
                            content_type="image/jpg")  # what if its not jpg
